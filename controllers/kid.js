...
exports.kid_create_post = async function(req, res) { 
    ...
    document.blood_group = req.body.memory; 
    try{ 
...
${JSON.stringify(req.body)}`) 
    try { 
        ...
        if(req.body.kid_type)  toUpdate.cost = req.body.cost; 
...
exports.kid_view_all_Page = async function(req, res) { 
    try{ 
        thekids = await kid.find(); 
        res.render('kid', { title: 'kid Search Results', results: thekids }); 
    } 
    catch(err){ 
        res.status(500); 
        res.send(`{"error": ${err}}`); 
